...
class Vehicle(object):
    ...
    def choose_next_state(self, predictions):
        ...
        best_next_trajectory = None
        min_cost = 999999999

        # Generate possible successor states in FSM
        possible_successor_states = self.successor_states()
        
        # Determine trajectory for each possible successor state
        for state in possible_successor_states:
            trajectory_for_state = self.generate_trajectory(state, predictions)

            # Calculate cost associated with trajectory
            cost = calculate_cost(self, trajectory_for_state, predictions)
            
            # Update best next trajectory if costs are lowest
            if cost < min_cost:
                best_next_trajectory = trajectory_for_state
                min_cost = cost
