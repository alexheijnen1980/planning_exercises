...
def goal_distance_cost(vehicle, trajectory, predictions, data):
    ...
    """
    if (data.end_distance_to_goal > 0):
        delta_d =  2.0 * vehicle.goal_lane - data.intended_lane - data.final_lane
        cost = 1 - exp (-(abs(delta_d / data.end_distance_to_goal)))
    else:
        cost = 1
...
def inefficiency_cost(vehicle, trajectory, predictions, data):
    ...
    """
    proposed_speed_intended = velocity(predictions, data.intended_lane)  
    if (proposed_speed_intended == None):
        proposed_speed_intended = vehicle.target_speed
    proposed_speed_final = velocity(predictions, data.final_lane)
    if (proposed_speed_final == None):
